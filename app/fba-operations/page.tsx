  ...
  return (
    <div style={{ background: '#ffffff' }}>
      ...
      <section style={{ minHeight: '55vh', display: 'flex', alignItems: 'center', background: 'linear-gradient(160deg, #030b1a 0%, #050d1f 50%, #081628 100%)', paddingTop: '8rem', paddingBottom: '5rem', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(14,165,176,0.1) 0%, transparent 70%)', top: -200, right: -100, pointerEvents: 'none' }} />
        <div style={wrap}>
        ...
        <div style={wrap}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'start' }}>
            <div>
              ...
              <p style={{ ...p, marginBottom: '1.25rem' }}>For brands and suppliers, this means that products purchased by Urban Pulse Wholesale LLC will be handled by Amazon's world-class logistics infrastructure — stored in secure, climate-controlled facilities and delivered to customers with Amazon's trusted service standards.</p>
            ...
            <div style={{ background: '#050d1f', borderRadius: 20, padding: '2rem' }}>
              <div style={{ color: '#d4a843', fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: '1.5rem', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>FBA Benefits for Supplier Partners</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', gap: '0.75rem', paddingBottom: '0.875rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    <CheckCircle2 size={15} color="#0ea5b0" style={{ flexShrink: 0, marginTop: 2 }} />
                    <div>
                      <div style={{ color: '#d4a843', fontSize: '0.75rem', fontWeight: 700, marginBottom: 2, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{item.label}</div>
                      <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.82rem', lineHeight: 1.6, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{item.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section style={{ padding: '6rem 0', background: '#f8f9fc' }}>
        <div style={wrap}>
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <p style={{ ...p, maxWidth: 500, margin: '0 auto', fontSize: '1rem' }}>From purchase order to customer delivery — a structured, professional process at every step.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            {[
              ...
              <div key={step.n} style={{ background: 'white', borderRadius: 16, padding: '1.75rem', border: '1px solid #e2e8f0', boxShadow: '0 2px 20px rgba(5,13,31,0.05)' }}>
                <div style={{ fontFamily: 'Playfair Display, serif', fontSize: '2.5rem', fontWeight: 900, background: 'linear-gradient(135deg, #d4a843, #0ea5b0)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', marginBottom: '0.75rem', lineHeight: 1 }}>{step.n}</div>
                <h3 style={{ color: '#050d1f', fontSize: '1rem', fontFamily: 'Playfair Display, serif', marginBottom: '0.6rem', fontWeight: 700 }}>{step.title}</h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {[
                  ...
                  <div key={item.title} style={{ display: 'flex', gap: '0.75rem' }}>
                    <CheckCircle2 size={15} color="#d4a843" style={{ flexShrink: 0, marginTop: 4 }} />
                    <div>
                      <div style={{ color: '#050d1f', fontWeight: 700, fontSize: '0.875rem', marginBottom: 3, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{item.title}</div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {[
                  ...
                  <div key={item.metric} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 0', borderBottom: '1px solid #f1f5f9', gap: '1rem' }}>
                    <div style={{ color: '#050d1f', fontWeight: 600, fontSize: '0.875rem', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{item.metric}</div>
                    <div style={{ color: '#64748b', fontSize: '0.82rem', textAlign: 'right', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{item.detail}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section style={{ padding: '6rem 0', background: 'linear-gradient(160deg, #030b1a 0%, #050d1f 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={wrap}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }}>
            <div>
              <div style={bar} />
              <h2 style={h2w}>Product Forecasting & Planning</h2>
              <p style={{ color: 'rgba(255,255,255,0.65)', lineHeight: 1.85, marginBottom: '1.25rem', fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: '1rem' }}>
                We don't make purchasing decisions arbitrarily. Our buying process is grounded in data — sales trends, market research, competitive analysis, and supplier input — so suppliers receive consistent, predictable ordering rather than erratic spikes.
              </p>
              <p style={{ color: 'rgba(255,255,255,0.55)', lineHeight: 1.85, marginBottom: '2rem', fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: '0.95rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              {[
                ...
                <div key={item.title} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '1.1rem' }}>
                  <div style={{ color: '#d4a843', fontWeight: 700, fontSize: '0.82rem', marginBottom: 4, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{item.title}</div>
                  <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.8rem', lineHeight: 1.6, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{item.desc}</div>
                </div>
              ))}
            </div>
